...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for style in qs:
            ...
            detected_products_info = self._detect_products(image_path)
            ...
            with transaction.atomic():
                for idx, prod_info in enumerate(detected_products_info):
                    ...
                    personal_attr = self._extract_personal_attributes(prod_info['image_crop'])
                    ...
                    sp = StylePredict.objects.create(
                        ...
                        crop_meta=json.dumps({
                            ...
                            'gender': personal_attr['gender'],
                            'age': personal_attr['age'],
                            'skin_color': personal_attr['skin_color'],
                            'hair_color': personal_attr['hair_color'],
                            'height': personal_attr['height'],
                            'body_type': personal_attr['body_type']
                        })
                    )
    ...
    def _detect_products(self, image_path):
    ...
    def _extract_personal_attributes(self, img_input):
        ...
        t = self.transform(img).unsqueeze(0).to(self.device)
        with torch.no_grad():
            feat = self.feature_extractor(t).squeeze(-1).squeeze(-1)
            gender = ['female','male'][torch.argmax(self.gender_model(feat)).item()]
            age = float(self.age_model(feat).item())
            skin_color = tuple(map(int, self.skin_model(feat).cpu().numpy()))
        ...
        return {'gender': gender,'age':age,'skin_color':skin_color,'hair_color':hair_color,'height':height,'body_type':body_type}
