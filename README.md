...
- این پروژه شامل فایل `docker-compose.yml` در ریشه است که سرویس `backend` را اجرا می‌کند و از پوشهٔ تولیدشدهٔ `frontend/build` برای سرو کردن فایل‌های استاتیک استفاده می‌نماید.
  1. اگر هنوز نسخهٔ تولید (production build) فرانت را ندارید، در پوشهٔ `frontend/` اجرا کنید:
  ...
  2. سپس در ریشهٔ پروژه، کانتینر را اجرا کنید:

  ```bash
  docker-compose up --build -d
  ```

- نکته: فایل `docker-compose.yml` به‌صورت پیش‌فرض فقط سرویس backend را راه‌اندازی می‌کند و `frontend/build` را به‌عنوان mount read-only مصرف می‌کند. اگر می‌خواهید فرانت نیز داخل کانتینر ساخته یا سرویس مجزایی برای آن اضافه شود (یا nginx/postgres اضافه شود)، اطلاع دهید تا نسخهٔ composeِ کامل‌تری بسازم.
